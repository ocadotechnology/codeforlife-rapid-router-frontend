 ...
 *
 * @param tileset The tile ID to fill the row with. Defaults to `Tilesets.EMPTY`
 *  if not provided.
...
>(options: FillRowOptions<ID, COLS> = {}) {
  const { tileset = 0 as ID, cols = COLS as COLS } = options
  return Array(cols).fill(tileset) as TileLayerRow<ID, COLS>
}
